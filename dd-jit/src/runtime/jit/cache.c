...
#define CLK int _th = g_threaded; if (_th) pthread_mutex_lock(&g_cache_lock)
#define CUL do { if (_th) pthread_mutex_unlock(&g_cache_lock); } while (0)
...
static struct { uint32_t *slot; uint64_t target; int is_bl; } g_pend[1 << 16];   // is_bl: §B host bl, patch as bl
static int g_npend;
static void add_pend2(uint32_t *slot, uint64_t target, int is_bl) {
    if (g_npend < (1 << 16)) { g_pend[g_npend].slot = slot; g_pend[g_npend].target = target; g_pend[g_npend].is_bl = is_bl; g_npend++; }
}
...
static void patch_links_to(uint64_t gpc, void *body) {
    for (int i = 0; i < g_npend;) {
        if (g_pend[i].target == gpc) {
            ...
            *g_pend[i].slot = (g_pend[i].is_bl ? 0x94000000u : 0x14000000u) | ((uint32_t)d & 0x3FFFFFFu);  // bl / b target.body
