...
static void e_stp(int t, int t2, int n, int off) { emit32(0xA9000000u | (((unsigned)(off / 8) & 0x7F) << 15) | (t2 << 10) | (n << 5) | t); }
static void e_ldp(int t, int t2, int n, int off) { emit32(0xA9400000u | (((unsigned)(off / 8) & 0x7F) << 15) | (t2 << 10) | (n << 5) | t); }
static void e_addi(int d, int n, unsigned imm) { emit32(0x91000000u | ((imm & 0xFFF) << 10) | (n << 5) | d); }       // add xd,xn,#imm
static void e_addlsl4(int d, int n, int m) { emit32(0x8B000000u | (m << 16) | (4 << 10) | (n << 5) | d); }           // add xd,xn,xm,lsl #4
static void e_addlsl3(int d, int n, int m) { emit32(0x8B000000u | (m << 16) | (3 << 10) | (n << 5) | d); }           // add xd,xn,xm,lsl #3
...
static void e_subi(int d, int n, unsigned imm) { emit32(0xD1000000u | ((imm & 0xFFF) << 10) | (n << 5) | d); }       // sub xd,xn,#imm
...
static void e_stur(int rt, int rn, int imm9) { emit32(0xF8000000u | (((unsigned)imm9 & 0x1FF) << 12) | (rn << 5) | rt); }
static void e_ldur(int rt, int rn, int imm9) { emit32(0xF8400000u | (((unsigned)imm9 & 0x1FF) << 12) | (rn << 5) | rt); }
static void e_str_q(int t, int rn, int off) { emit32(0x3D800000u | (((unsigned)off / 16) << 10) | (rn << 5) | t); } // str q,[xn,#off]
static void e_ldr_q(int t, int rn, int off) { emit32(0x3DC00000u | (((unsigned)off / 16) << 10) | (rn << 5) | t); } // ldr q,[xn,#off]
...
static void emit_prologue(void) {
    e_ldr(9, 0, OFF_SP); e_mov_sp_from(9);
    e_ldr(9, 0, OFF_NZCV); emit32(0xD51B4200u | 9);          // msr nzcv, x9 (restore flags)
    for (int t = 0; t < 32; t += 2) e_ldp_q(t, t + 1, 0, OFF_V + t * 16);  // guest V0..V31 (paired)
    for (int r = 1; r <= 30; r++) if (!is_stolen(r)) e_ldr(r, 0, r * 8);  // x18,x28 stolen: live only in cpu->x[]
...
static void emit_spill(void) {
    ...
    emit32(0xD53B4200u | 0); e_stur(0, 31, -24);             // mrs x0, nzcv; stash flags (none of this clobbers flags)
    e_load_cpu(0);                                           // x0 = cpu (thread-local)
    for (int t = 0; t < 32; t += 2) e_stp_q(t, t + 1, 0, OFF_V + t * 16);  // guest V0..V31 (paired)
    for (int r = 1; r <= 30; r++) if (!is_stolen(r)) e_str(r, 0, r * 8);  // skip x18 (volatile) + x28 (= cpu)
    e_ldur(9, 31, -16); e_str(9, 0, 0);                       // cpu->x[0] = saved guest x0
    e_ldur(9, 31, -24); e_str(9, 0, OFF_NZCV);               // cpu->nzcv
    e_mov_from_sp(9); e_str(9, 0, OFF_SP);                    // cpu->sp
}
static void emit_exit_const(uint64_t pc, uint64_t reason) {
    emit_spill();                                             // x0 = cpu
    e_movconst(9, pc); e_str(9, 0, OFF_PC);
    e_movconst(9, reason); e_str(9, 0, OFF_RSN);
    e_movconst(9, (uint64_t)block_return); e_br(9);           // x0=cpu -> block_return
}
static void emit_exit_reg(int rn, uint64_t reason) {
    emit_spill();                                             // x0 = cpu
    e_ldr(9, 0, rn * 8); e_str(9, 0, OFF_PC);                 // cpu->pc = cpu->x[rn]
    e_movconst(9, reason); e_str(9, 0, OFF_RSN);
...
static void emit_ibtc_miss(int rn) {                          // shared IBTC miss tail (x16/x17 restored)
    ...
    e_ldr(9, 0, rn * 8); e_str(9, 0, OFF_PC);                 // cpu->pc = guest target
    e_movconst(9, R_BRANCH); e_str(9, 0, OFF_RSN);
    e_movconst(9, 1); e_str(9, 0, OFF_ICSITE);                // flag: indirect miss -> insert into IBTC
    e_movconst(9, (uint64_t)block_return); e_br(9);
}
static void emit_ibranch(int rn) {
    if (rn == 18 || rn == 28) { emit_exit_reg(rn, R_BRANCH); return; }  // host x18 volatile / x28=cpu: can't hold target
    if (rn == 30) e_ldr(30, CPUREG, 30 * 8);   // ret/br/blr x30: load guest x30 into the FREE host link reg, then
    ...
    if (rn == 16 || rn == 17) {
        ...
        e_stur(16, 31, -16); e_stur(17, 31, -24);
        // --- per-site monomorphic fast path: xRn (target) still live here ---
        uint32_t *p_ldrt = (uint32_t *)g_cp; emit32(0);       // ldr xOTHER, Lsite_tgt
        emit32(0xCB000000u | (rn << 16) | (other << 5) | other); // sub xOTHER, xOTHER, xRn
        uint32_t *p_cbslow = (uint32_t *)g_cp; emit32(0);     // cbnz xOTHER, Lslow
        ...
        uint32_t *p_cbnz = (uint32_t *)g_cp; emit32(0);       // cbnz x17, miss
        ...
        emit_spill();
        e_ldr(9, 0, rn * 8); e_str(9, 0, OFF_PC);
        e_movconst(9, R_BRANCH); e_str(9, 0, OFF_RSN);
        uint32_t *p_adr = (uint32_t *)g_cp; emit32(0);        // adr x9, Lsite_tgt -> dispatcher fills both caches
    ...
    e_stur(16, 31, -16); e_stur(17, 31, -24);                 // stash x16/x17 scratch
    // --- per-site monomorphic cache (ahead of the shared hash; ret/br/blr are usually monomorphic) ---
    uint32_t *p_ldrt = (uint32_t *)g_cp; emit32(0);           // ldr x16, Lsite_tgt
    emit32(0xCB000000u | (rn << 16) | (16 << 5) | 16);        // sub x16, x16, xRn   (xRn live; rn != 16/17 here)
    uint32_t *p_cbslow = (uint32_t *)g_cp; emit32(0);         // cbnz x16, Lhash
    uint32_t *p_ldrb = (uint32_t *)g_cp; emit32(0);           // ldr x16, Lsite_body
    e_br(16);                                                 // HIT -> body_ind (restores x16/x17)
    ...
    e_br(16);                                                 // HIT -> jump
    uint32_t *miss = (uint32_t *)g_cp;
    e_ldur(16, 31, -16); e_ldur(17, 31, -24);                 // restore scratch
    emit_spill();                                             // slow path: x0 = cpu
    e_ldr(9, 0, rn * 8); e_str(9, 0, OFF_PC);                 // cpu->pc = guest target
    e_movconst(9, R_BRANCH); e_str(9, 0, OFF_RSN);
    uint32_t *p_adr = (uint32_t *)g_cp; emit32(0);            // adr x9, Lsite_tgt -> dispatcher fills the per-site cache
    e_str(9, 0, OFF_ICSITE);
    e_movconst(9, (uint64_t)block_return); e_br(9);
    if ((uint64_t)g_cp & 7) emit32(0);
    uint8_t *Lt = g_cp; *(uint64_t *)g_cp = 0; g_cp += 8;     // Lsite_tgt
    uint8_t *Lb = g_cp; *(uint64_t *)g_cp = 0; g_cp += 8;     // Lsite_body
