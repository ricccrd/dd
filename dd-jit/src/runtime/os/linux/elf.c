...
static int elf_interp(const char *path, char *out, size_t n) {
    ...
    for (int i = 0; i < phnum; i++) {
        ...
        if (rd32(ph) == 3) { uint64_t off = rd64(ph + 8), fsz = rd64(ph + 32);   // PT_INTERP
...
static void load_elf(const char *path, struct loaded *out) {
    ...
    if (getenv("JT")) fprintf(stderr, "[LOADED] %s base=%llx entry=%llx\n", path, (unsigned long long)base, (unsigned long long)out->entry);
    out->phdr = (uint64_t)base + phoff;          // phdrs live at file offset phoff in seg 0
    out->phent = phentsize; out->phnum = phnum;
...
static char *g_guest_env[] = {
    "PATH=/usr/bin:/bin", "HOME=/root", "TERM=dumb", "LANG=C",
    "GLIBC_TUNABLES=glibc.cpu.aarch64_gcs=0", NULL,
};
static uint64_t build_stack(int argc, char **argv, struct loaded *lm, uint64_t at_base) {
    ...
    top -= 8; memcpy(top, "aarch64", 8); uint64_t plat = (uint64_t)top;
    top -= 16; arc4random_buf(top, 16); uint64_t rnd = (uint64_t)top;
    top = (uint8_t *)((uint64_t)top & ~15ull);
    uint64_t aux[][2] = {
        {3, lm->phdr}, {4, (uint64_t)lm->phent}, {5, (uint64_t)lm->phnum}, {6, 16384},
        {7, at_base}, {8, 0}, {9, lm->entry}, {11, (uint64_t)cuid()}, {12, (uint64_t)cuid()}, {13, (uint64_t)cgid()},
        {14, (uint64_t)cgid()}, {16, 0x1fb}, {17, 100}, {15, plat}, {25, rnd}, {23, 0},
        {31, argc ? argp[0] : 0}, {0, 0},
    };
    ...
    *p++ = 0;
    ...
    for (int i = 0; i < naux && g_auxv_len + 16 <= (int)sizeof g_auxv_data; i++) {
        memcpy(g_auxv_data + g_auxv_len, &aux[i][0], 8); memcpy(g_auxv_data + g_auxv_len + 8, &aux[i][1], 8); g_auxv_len += 16;
    }
    return (uint64_t)sp;
}
