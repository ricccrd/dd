// dd/runtime/os/linux/container -- container config state (UTS/cgroup/USER-ns/port-map) + parsers.

// ---- container namespace + cgroup state (SentryConfig: ddockerd -> jit) ----
static char     g_hostname[65] = "";          // UTS ns: container hostname (uname/sethostname); "" = host default
static uint64_t g_mem_max = 0;                // cgroup memory.max bytes (0 = unlimited); charged in mmap
static int      g_pids_max = 0;               // cgroup pids.max (0 = unlimited); checked in clone
static _Atomic uint64_t g_mem_charged = 0;    // current anon charge (bytes)
static _Atomic int      g_pids_cur = 1;       // live task count (init = 1)
static int      g_init_hostpid = 0;           // PID ns: host pid of the container init -> guest sees it as PID 1
static int container_pid(void) { int h = getpid(); return (g_init_hostpid && h == g_init_hostpid) ? 1 : h; }
static int g_uid = -1, g_gid = -1;            // USER ns: container uid/gid (-1 = passthrough host id; container defaults to 0=root)
static int cuid(void) { return g_uid >= 0 ? g_uid : (int)getuid(); }
static int cgid(void) { return g_gid >= 0 ? g_gid : (int)getgid(); }
// ---- NET ns Phase 1: port-map (docker run -p H:C). bind(:C) actually binds the host port :H;
// getsockname reports :C back so the guest sees the port it asked for. {cport->hport} table.
static struct { uint16_t cport, hport; } g_portmap[32];
static int g_nportmap = 0;
static uint16_t g_fd_cport[1024];                       // fd -> the container port it bound (for getsockname)
static uint16_t pm_host(uint16_t c) { for (int i = 0; i < g_nportmap; i++) if (g_portmap[i].cport == c) return g_portmap[i].hport; return c; }
static void parse_publish(const char *s) {              // "H:C,H:C,..." (docker -p order: host:container)
    while (s && *s && g_nportmap < 32) {
        int h = atoi(s); const char *colon = strchr(s, ':'); if (!colon) break;
        int cc = atoi(colon + 1); if (h > 0 && cc > 0) { g_portmap[g_nportmap].cport = (uint16_t)cc; g_portmap[g_nportmap].hport = (uint16_t)h; g_nportmap++; }
        const char *comma = strchr(s, ','); if (!comma) break; s = comma + 1;
    }
}
static uint64_t parse_size(const char *s) {   // "128M"/"2G"/"512K"/"1048576" -> bytes (docker-style suffixes)
    if (!s) return 0; char *e; uint64_t v = strtoull(s, &e, 10);
    switch (*e) { case 'k': case 'K': return v << 10; case 'm': case 'M': return v << 20;
                  case 'g': case 'G': return v << 30; default: return v; }
}

// guest PC -> (host = prologue entry for a fresh dispatcher entry,
//             body = post-prologue entry for a CHAINED jump with regs already live)
#define MAP_N 65536
