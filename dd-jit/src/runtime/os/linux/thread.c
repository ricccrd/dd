...
static long futex_op(int *uaddr, int op, int val, const struct timespec *ts) {
    if (op == 0 || op == 9) {                        // FUTEX_WAIT / WAIT_BITSET: sleep while *uaddr == val
        ...
        if (__atomic_load_n(uaddr, __ATOMIC_SEQ_CST) != val) { pthread_mutex_unlock(&g_futex_m); return -EAGAIN; } // EAGAIN
        if (ts) { struct timespec abs; clock_gettime(CLOCK_REALTIME, &abs);
                  abs.tv_sec += ts->tv_sec; abs.tv_nsec += ts->tv_nsec;
                  if (abs.tv_nsec >= 1000000000) { abs.tv_sec++; abs.tv_nsec -= 1000000000; }
                  pthread_cond_timedwait(&g_futex_c, &g_futex_m, &abs); }
        else pthread_cond_wait(&g_futex_c, &g_futex_m);
...
static void futex_wake_addr(uint64_t uaddr) {
    ...
    *(int *)uaddr = 0;                               // CLONE_CHILD_CLEARTID: zero then wake joiners
    pthread_mutex_lock(&g_futex_m); pthread_cond_broadcast(&g_futex_c); pthread_mutex_unlock(&g_futex_m);
}
...
static int spawn_thread(struct cpu *parent, uint64_t flags, uint64_t stack_top,
    ...
    *child = *parent;
    ...
    child->exited = 0; child->redirect = 0;
    ...
    g_threaded = 1;
    pthread_t th;
    if (pthread_create(&th, NULL, thread_trampoline, child) != 0) { free(child); return -EAGAIN; }
