...
static struct { uint64_t handler, flags, mask; } g_sigact[65];
...
static void build_signal_frame(struct cpu *c, int sig) {
    if (g_trace) fprintf(stderr, "[sig] deliver %d sp=%llx handler=%llx\n", sig, (unsigned long long)c->sp, (unsigned long long)g_sigact[sig].handler);
    uint64_t frame = (c->sp - 4688) & ~15ull;
    uint8_t *f = (uint8_t *)frame; memset(f, 0, 4688);
    *(int *)(f + 0) = sig;                                  // siginfo.si_signo
    uint64_t uc = frame + 128, mc = uc + 168;
    *(uint64_t *)(uc + 40) = c->sigmask;                   // uc_sigmask (signal mask to restore)
    for (int i = 0; i < 31; i++) *(uint64_t *)(mc + 8 + i * 8) = c->x[i];
    ...
    *(uint64_t *)(mc + 272) = c->nzcv;
    ...
    c->x[0] = (uint64_t)sig; c->x[1] = frame; c->x[2] = uc; // handler(signo, siginfo*, ucontext*)
    c->x[30] = SIGRETURN_PC;                                // return address -> sigreturn
    c->sp = frame; c->pc = g_sigact[sig].handler;
...
static void do_sigreturn(struct cpu *c) {
    ...
    for (int i = 0; i < 31; i++) c->x[i] = *(uint64_t *)(mc + 8 + i * 8);
    c->sp = *(uint64_t *)(mc + 256); c->pc = *(uint64_t *)(mc + 264);
...
static void maybe_deliver_signal(struct cpu *c) {
    ...
    for (int sig = 1; sig <= 64; sig++) {
        ...
        if (h <= 1) { __atomic_and_fetch(&g_pending, ~bit, __ATOMIC_SEQ_CST); continue; } // DFL/IGN: host did it
...
static void raise_guest_signal(struct cpu *c, int sig) {
    ...
    c->exited = 1; c->exit_code = 128 + sig;                             // fallback if raise returns / signo invalid on host
}
