...
static int gpr_field_mask(uint32_t in) {
    ...
    if ((in & 0x0E000000u) == 0x0A000000u) {                         // Data-processing register
        ...
        if ((in & 0x1FE00000u) == 0x1A400000u)
            return (in & 0x800u) ? 2 : (2 | 4); // ccmp/ccmn: [4:0]=nzcv; imm -> Rn only
...
static void emit_mangled_x18(uint32_t in, int mask) {
    ...
    for (int i = 0; i < ns; i++)
        e_str(sc[i], CPUREG, (int)OFF_MSCRATCH + 8 * i); // spill scratch -> cpu->mscratch
    for (int i = 0; i < ns; i++)
        e_ldr(sc[i], CPUREG, stolen[i] * 8); // scratch = cpu->x[stolen]
    uint32_t m = in;
    for (int k = 0; k < 4; k++)
        if (mask & mbits[k]) {
    ...
    emit32(m);
    for (int i = 0; i < ns; i++)
        e_str(sc[i], CPUREG, stolen[i] * 8); // cpu->x[stolen] = scratch
    for (int i = 0; i < ns; i++)
        e_ldr(sc[i], CPUREG, (int)OFF_MSCRATCH + 8 * i); // restore scratch
}
...
static uint32_t *emit_shadow_push(uint64_t gpc) {
    ...
    e_stp(0, 1, CPUREG, M);
    e_stp(2, 3, CPUREG, M + 16); // spill x0..x3 -> mscratch (paired: 2 stp not 4 str)
    e_movconst(0, gpc + 4);
    e_str(0, CPUREG, 30 * 8);       // x0 = guest_ret; cpu->x[30] = guest_ret (ALWAYS)
    e_ldr(1, CPUREG, (int)OFF_SSP); // x1 = ssp (capped at 1024)
    uint32_t *p_full = (uint32_t *)g_cp;
    emit32(0); // tbnz x1, #10, Lskip (ssp==1024 -> overflow; no flags)
    e_addlsl4(2, CPUREG, 1);
    e_addi(2, 2, (unsigned)OFF_SSTK); // x2 = C + idx*16 + OFF_SSTK = &sstk[2*ssp]
    uint32_t *p_adr = (uint32_t *)g_cp;
    emit32(0);         // adr x3, Lcont (host_ret; backpatched)
    e_stp(0, 3, 2, 0); // sstk[2*ssp] = (guest_ret, host_ret=&Lcont)
    e_mov_from_sp(3);
    e_addlsl3(2, CPUREG, 1);
    e_str(3, 2, (int)OFF_GSP); // gsp[ssp] = current guest SP (frame disambiguator)
    e_addi(1, 1, 1);
    e_str(1, CPUREG, (int)OFF_SSP); // ssp++
    uint8_t *Lskip = g_cp;
    *p_full = 0x37000000u | (10u << 19) | (((uint32_t)(((uint8_t *)Lskip - (uint8_t *)p_full) / 4) & 0x3FFF) << 5) |
              1; // tbnz x1,#10
    e_ldp(0, 1, CPUREG, M);
    e_ldp(2, 3, CPUREG, M + 16); // restore x0..x3 (paired: 2 ldp not 4 ldr)
    return p_adr;
}
...
static int target_is_leaf(uint64_t target) {
    ...
    for (int i = 0; i < n && i < 16; i++)
        if (!is_leaf0(calls[i])) return 0; // calls a non-leaf -> deep -> §B
...
static void emit_shadow_ret(void) {
    ...
    emit32(0); // cbnz x1, Lfb (foreign/longjmp)
    e_addlsl3(1, CPUREG, 0);
    e_ldr(2, 1, (int)OFF_GSP);                      // x2 = gsp[idx] (guest SP captured at the bl)
    ...
    emit32(0);                      // cbnz x1, Lfb (guest_ret matched but wrong frame -> slow)
    e_str(0, CPUREG, (int)OFF_SSP); // FAST: ssp-- (pop)
    e_movr(30, 3);                  // real x30 = host_ret
    e_ldp(0, 1, CPUREG, M);
    e_ldp(2, 3, CPUREG, M + 16); // restore x0..x3 (paired)
...
static void emit_ret_ic(void) {
    e_stur(16, 31, -16);
    e_stur(17, 31, -24);       // stash scratch (target body_ind restores it)
    e_ldr(16, CPUREG, 30 * 8); // x16 = cpu->x[30]
    uint32_t *p_ldrt = (uint32_t *)g_cp;
    emit32(0);                                         // ldr x17, Lsite_tgt
    ...
    emit_spill();
    e_ldr(9, 0, 30 * 8);
    e_str(9, 0, OFF_PC); // cpu->pc = cpu->x[30]
    e_movconst(9, R_BRANCH);
    e_str(9, 0, OFF_RSN);
    uint32_t *p_adr = (uint32_t *)g_cp;
    emit32(0); // adr x9, Lsite_tgt -> dispatcher fills the site
...
static int try_lse_atomic(uint64_t gpc) {
    ...
    if (op >= 0) {
        ...
        if (Wm >= 0 && (i2 & 0x3F400000u) == 0x08000000u && ((i2 >> 30) & 3) == sz && (i2 & 31) == Ws2 &&
            ...
            if ((i3 & 0xFF000000u) == 0x35000000u && (i3 & 31) == Ws &&
                ...
                emit32(rec);
                g_lse_n++; // reconstruct new value (re-emit original op)
                return 16;
            }
        }
    }
    ...
    if ((i1 & 0xFFC00000u) == addib && ((i1 >> 5) & 31) == Wt) {
        ...
        if ((i2 & 0x3F400000u) == 0x08000000u && ((i2 >> 30) & 3) == sz && (i2 & 31) == Ws2 && ((i2 >> 5) & 31) == Xn &&
            ...
            if ((i3 & 0xFF000000u) == 0x35000000u && (i3 & 31) == Ws &&
                ...
                e_lse(0xB8E00000u, sz, Ws, Wt, Xn); // ldaddal Ws, Wt, [Xn]
                emit32(i1);
                g_lse_n++; // re-emit add Ws2, Wt, #imm (reconstruct)
                return 16;
            }
        }
    }
    ...
    if ((i1 & 0xFFE0FC1Fu) == subsb && ((i1 >> 5) & 31) == Wt) {
        ...
        if ((i2 & 0xFF00001Fu) == 0x54000001u // b.ne
            ...
            g_lse_n++; // cmp Wt, Wexp (reproduce NZCV)
            return 20;
        }
    }
    return 0;
...
static void *translate_block(uint64_t gpc) {
    ...
    for (;;) {
        ...
        if ((in & 0x3F400000u) == 0x08400000u)
            in_excl = 1; // load-exclusive
        else if (in_excl && (in & 0x3F400000u) == 0x08000000u)
            in_excl = 0; // store-exclusive
        ...
        if ((in & 0xFC000000u) == 0x94000000u) { // bl
            ...
            break; // §B: shadow push + host bl (RAS) + Lcont continuation
        }
        if ((in & 0xFFFFFC1Fu) == 0xD65F0000u) { // ret xN
            int rrn = (in >> 5) & 31;
            if (rrn == 30)
                emit_shadow_ret(); // §B: FAST shadow ret (guest_ret+guest_sp match -> host ret) / IBTC fallback
            else
                emit_ibranch(rrn); // ret xN via another reg -> ordinary indirect branch
            break;
        }
        ...
        if ((in & 0xFFFFFC1Fu) == 0xD63F0000u) { // blr
            ...
            emit_ibranch((in >> 5) & 31);
            break; //   (Section 3) -- deferred; Stage-B IBTC for the function-ptr return
        }
        if ((in & 0xFF000010u) == 0x54000000u) { // b.cond
            ...
            emit32(0); // b.cond -> taken (backpatched)
        ...
        if ((in & 0x7E000000u) == 0x34000000u) { // cbz / cbnz
            ...
            if (is_stolen(rt)) { // tested reg stolen -> test cpu->x[rt] via a saved scratch
                ...
                e_ldr(S, CPUREG, rt * 8);
                uint32_t *patch = (uint32_t *)g_cp;
                emit32(0); // cbz/cbnz S -> taken
                e_ldr(S, CPUREG, (int)OFF_MSCRATCH);
                emit_chain_exit(fall); // fall: restore S
                int64_t d = ((uint8_t *)g_cp - (uint8_t *)patch) / 4;
                *patch = 0x34000000u | ((unsigned)sf << 31) | ((unsigned)op << 24) | ((uint32_t)(d & 0x7FFFF) << 5) | S;
                e_ldr(S, CPUREG, (int)OFF_MSCRATCH);
                emit_chain_exit(taken);
                break; // taken: restore S
            }
            uint32_t *patch = (uint32_t *)g_cp;
            emit32(0); // cbz/cbnz rt -> taken
            emit_chain_exit(fall);
            int64_t d = ((uint8_t *)g_cp - (uint8_t *)patch) / 4;
        ...
        if ((in & 0x7E000000u) == 0x36000000u) { // tbz / tbnz
            ...
            uint32_t *patch = (uint32_t *)g_cp;
            emit32(0); // tbz/tbnz rt,#bit -> taken
            emit_chain_exit(fall);
            int64_t d = ((uint8_t *)g_cp - (uint8_t *)patch) / 4;
            *patch = 0x36000000u | ((unsigned)bit5 << 31) | ((unsigned)op << 24) | ((unsigned)b40 << 19) |
        ...
        if (uses_x18(in, mask))
            emit_mangled_x18(in, mask); // unless it names x18 -> mangle
        else
            emit32(in);
        gpc += 4;
    }
